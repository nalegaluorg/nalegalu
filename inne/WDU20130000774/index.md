---
title: Rozporządzenie Ministra Finansów z dnia 1 lipca 2013 r. w sprawie wzoru certyfikatu weryfikacji dostawy oraz sposobu ewidencjonowania wydanych certyfikatów
address: WDU20130000774
eli: DU/2013/774
publisher: DU
year: 2013
pos: 774
act_type: Rozporządzenie
status: akt posiada tekst jednolity
domain: null
source: "isap-pdf"
---

# RZECZYPOSPOLITEJ POLSKIEJ Warszawa, dnia 4 lipca 2013 r.

| Pole | Wartość |
|---|---|
| Adres publikacyjny | Dz.U. 2013 poz. 774 |
| ISAP | WDU20130000774 |
| Typ aktu | Rozporządzenie |
| Status | akt posiada tekst jednolity |
| W mocy | tak |
| Data ogłoszenia | 2013-07-01 |
| Data wydania | 2013-07-04 |
| Ostatnia zmiana | 2024-03-14 |
| Źródło | [ISAP](https://isap.sejm.gov.pl/isap.nsf/DocDetails.xsp?id=WDU20130000774) · [PDF](https://api.sejm.gov.pl/eli/acts/DU/2013/774/text.pdf) |

ROZPORZĄDZENIE MINISTRA FINANSÓW dnia 1 lipca 2013 r.
w sprawie wzoru certyfikatu weryfikacji dostawy oraz sposobu ewidencjonowania wydanych certyfikatów Na art. 24 ust. 5 ustawy dnia 29 listopada 2000 r. o obrocie zagranicą towarami, technologiami i usługami o znaczeniu strategicznym dla bezpieczeństwa państwa, a także dla utrzymania międzynarodowego pokoju i bezpieczeństwa (Dz. U. 2013 r. 194) zarządza się, co następuje: § 1. Określa się wzór certyfikatu weryfikacji dostawy, stanowiący załącznik rozporządzenia.
§ 2. Ewidencję wydanych certyfikatów weryfikacji dostawy, zwaną dalej „ewidencją”, prowadzi naczelnik urzędu celnego, w którym może być dokonywany wywóz, przywóz lub tranzyt towarów o znaczeniu strategicznym.
§ 3. 1. Podstawą wpisów ewidencji są dane zawarte w certyfikatach weryfikacji dostawy.
2. Każdy wpis ewidencji oznacza się numerem ewidencyjnym wynikającym kolejności wpisów oraz zaopatruje w datę dokonania wpisu i osoby prowadzącej ewidencję.
§ 4. Ewidencja zawiera następujące dane:
1) numer ewidencyjny;
2) numer certyfikatu importowego;
3) nazwę importera lub odbiorcy, jego siedzibę i adres;
4) nazwę eksportera lub dostawcy, jego siedzibę i adres;
5) nazwę i opis towaru o znaczeniu strategicznym;
6) numer kontrolny towaru;
7) ilość i wartość towaru;
8) opis ostatecznego wykorzystania towaru;
9) numer zgłoszenia celnego;
10) numer listu przewozowego lub innego dokumentu potwierdzającego przywóz lub transfer wewnątrzunijny towaru o znaczeniu strategicznym;
11) datę wydania certyfikatu weryfikacji dostawy;
12) datę dokonania wpisu;
2. Poprawki powinny być dokonywane w sposób czytelny.
3. Na końcu wpisu sporządza się adnotację o treści poprawki, jej dacie i podstawie, ze szczególnym wskazaniem miejsca dokonanej poprawki, oraz opatruje adnotację podpisem.
§ 6. 1. Ewidencja może być prowadzona w formie papierowej lub w formie elektronicznej.
2. Ewidencja prowadzona w formie elektronicznej powinna umożliwiać:
1) wgląd w treść dokonywanych wpisów;
2) ochronę przechowywanych danych przed zniekształceniem, utratą lub nieuprawnioną modyfikacją;
3) identyfikację osoby dokonującej wpisu;
4) dokonywanie poprawek wpisu oraz sporządzanie adnotacji, o których mowa w § 5 ust. 3;
5) uzyskanie czytelnego wydruku wpisów w porządku chronologicznym.
§ 7. Rozporządzenie wchodzi w życie dniem następującym po dniu ogłoszenia.
Minister Finansów: wz. J. Cichoń Rzeczpospolita Polska CERTYFIKAT WERYFIKACJI DOSTAWY WZÓR Załącznik nr …… CERTYFIKATU WERYFIKACJI DOSTAWY

