...
- **3)** w art. 38a: w ust. 3 w pkt 1 lit. d otrzymuje brzmienie: „ stanu chemicznego jednolitych części wód powierzchniowych i środowiskowe normy jakości dla substancji priorytetowych określonych w przepisach wydanych na podstawie art. 38l ust. 2 oraz dla innych zanieczyszczeń, służące klasyfikacji tego stanu; ” , ust. 5–5b otrzymują brzmienie: „
    - **a)**
    - **d)**
    - **b)**
...
- **5)** przy określaniu poziomów tła następujące zasady: poziomy tła powinny być określane na podstawie charakterystyki jednolitych części wód podziemnych oraz na podstawie wyników monitoringu wód podziemnych, monitoring wód podziemnych i interpretacja danych powinny uwzględnić fakt, że warunki przepływu i skład chemiczny wód podziemnych są zróżnicowane w pionie i w poziomie, jeżeli dostępne są tylko ograniczone dane z monitoringu wód podziemnych, konieczne jest zebranie większej ilości danych, a do tego czasu poziomy tła powinny być określane w oparciu o te ograniczone dane z monitoringu, w stosownych przypadkach za pomocą uproszczonego podejścia z wykorzystaniem podzbioru próbek, dla których wskaźniki wykazują brak wpływu działalności człowieka, informacje dotyczące transferów i procesów geochemicznych również powinny być wzięte pod uwagę, jeżeli są one dostępne, jeżeli nie są dostępne wystarczające dane z monitoringu wód podziemnych, a informacje dotyczące transferów i procesów geochemicznych są niskiej jakości, konieczne jest zebranie większej ilości danych, a do tego czasu poziomy tła powinny być szacowane, w stosownych przypadkach na podstawie statystycznych wyników referencyjnych dla tego samego rodzaju warstw wodonośnych na innych obszarach, na których dane z monitoringu są wystarczające;
    - **a)**
    - **b)**
    - **c)**
    - **d)**
...
- **8)** w art. 88f: ust. 5 i 6 otrzymują brzmienie: „
    - **a)**
...
- **11)** w art. 113 w ust. 3 w pkt 1a lit. a otrzymuje brzmienie: „ substancji priorytetowych określonych w przepisach wydanych na podstawie art. 38l ust. 2, ” ;
    - **a)**
