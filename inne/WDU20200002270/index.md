...
§ 4. 1. Rejestr jest prowadzony w formie elektronicznej bazy danych dostępnej dla starostów i wójtów (burmistrzów, prezydentów miast).
...
Należy podać inne dane lokalizacyjne, w szczególności jak najbliższe osiedla, sołectwa, przysiółki.
Należy podać jedną z form terenu:
a) lej źródłowy,
...
i) inna.
Należy podać jeden z rodzajów układu warstw geologicznych względem stoku, na którym występuje koluwium:
a) asekwentne,
b) konsekwentne,
c) insekwentne,
d) obsekwentne,
e) subsekwentne,
f) złożone.
Należy podać jeden z rodzajów materiału budującego koluwia:
a) gruntowe (ziemne),
b) zwietrzeliny na skalnym podłożu (zwietrzelinowe),
c) skalne,
d) skalno-zwietrzelinowe,
e) mieszane.
Należy podać jeden z rodzajów ruchu:
a) obryw,
b) zsuw,
c) zsuw translacyjny,
d) zsuw rotacyjny,
...
Należy podać wysokość bezwzględną najwyżej położonego punktu w obrębie osuwiska (H ).
Należy podać wysokość bezwzględną najniżej położonego punktu w obrębie osuwiska (H ).
Należy podać różnicę wysokości pomiędzy najwyżej i najniżej położonymi i punktami w obrębie (R).
Należy podać średnie nachylenie osuwiska (α).
Należy podać maksymalną wysokość względną skarpy głównej (hsg).
Należy wpisać TAK lub NIE.
Mierzoną lub szacowaną miąższość koluwium należy podać w odpowiedniej rubryce.
Należy określić według Szczegółowej Mapy Geologicznej Polski (SMGP).
Należy określić sposób zalegania warstw w podłożu terenu, spośród następujących sposobów:
a) poziome,
...
W przypadku możliwości dokonania pomiaru zapadania nienaruszonych warstw w bezpośrednim sąsiedztwie należy podać uzyskane wartości (kierunek kąta upadu lub kąt upadu).
Należy podać z listy określenie oddające uwarunkowania tektoniczne spośród określeń:
a) obszar nasunięcia,
...
Należy podać rodzaj materiału koluwialnego spośród rodzajów:
a) detrytyczne,
b) pakietowe,
...
W celu określenia przejawów wód powierzchniowych i gruntowych w obrębie koluwium, skarpy głównej i stoku powyżej skarpy głównej, stoku poniżej osuwiska oraz stoku po bokach osuwiska należy podać jedną lub więcej, spośród określeń:
a) brak,
b) podmokłości,
c) młaki,
d) wysięki,
...
W odpowiedniej rubryce należy wstawić „X”.
W odpowiedniej rubryce należy podać liczbę obiektów.
W przypadku gdy na obszarze osuwiska występuje infrastruktura drogowa, należy podać jej rodzaj spośród określeń:
a) droga gminna,
b) droga powiatowa,
c) droga wojewódzka,
d) droga krajowa,
e) droga inna.
W przypadku gdy na obszarze osuwiska występuje infrastruktura kolejowa, należy wstawić „X”.
Należy podać liczbę, długość, powierzchnię i rodzaj uszkodzonych obiektów, obszarów (np. 2 budynki mieszkalne, 3 ha sadu, 50 m drogi) i ewentualnie krótki opis zniszczeń i uszkodzeń wywołanych przez osuwisko.
Należy podać liczbę, długość, powierzchnię i rodzaj zagrożonych obiektów i obszarów.
Wskazanie należy zaznaczyć „X”. Przy wskazaniu TAK należy podać opis wykonanych prac.
Wskazanie należy zaznaczyć „X”. Przy wskazaniu TAK należy podać opis przesłanek do prowadzenia wizji w terenie oraz proponowany sposób jej prowadzenia.
Wskazanie należy zaznaczyć „X”. Przy wskazaniu TAK należy podać opis przesłanek do prowadzenia monitoringu oraz proponowany sposób jego prowadzenia.
Należy podać informacje o dotychczasowym stanie badań osuwiska i wykonanych dokumentacjach w układzie chronologicznym.
Należy podać notki bibliograficzne prac oraz informacje dotyczące numeru(-ów) dokumentacji i miejsca ich przechowywania.
