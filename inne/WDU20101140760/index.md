...
# 2009 r. 18, poz. 97) wprowadza następujące zmiany:
...
1) 5 1:
a) pkt 3 otrzymuje brzmienie: „3) oznakowany, zastrzeżeniem 4, znakiem budowlanym, którego wzór określa załącznik nr 1 do niniejszej ustawy, albo”,
b) dodaje pkt 4 brzmieniu: „4) wprowadzony do obrotu legalnie innym państwie członkowskim Unii Europejskiej, nieobjęty zakresem przedmiotowym norm zharmonizowanych lub wytycznych do europejskich aprobat technicznych Europejskiej Organizacji do spraw Aprobat Technicznych (EOTA), jeżeli jego właściwości użytkowe umożliwiają spełnienie wymagań podstawowych przez obiekty budowlane zaprojektowane budowane sposób określony odrębnych przepisach, tym przepisach techniczno-budowlanych, zgodnie zasadami wiedzy technicznej.”;
2) 13:
a) 1 pkt 5 otrzymuje brzmienie: „5) przekazywanie Głównemu Inspektorowi Nadzoru Budowlanego informacji przeprowadzonych kontrolach lanych wprowadzonych do obrotu wydanych postanowieniach decyzjach, a także wydanych opiniach, których mowa pkt 6;”,
b) dodaje 5 brzmieniu: „5. Wojewódzki inspektor nadzoru budowlanego może wezwać powiatowego inspektora nadzoru budowlanego do przeprowadzenia kontroli wskazanej budowy lub robót budowlanych zakresie stosowania określonych budowlanych.”;
3) 14:
a) 1 dodaje pkt 5 6 brzmieniu: „5) gromadzenie informacji dotyczących przeprowadzonych kontroli budowlanych wprowadzonych do obrotu wydanych postanowieniach, decyzjach opiniach;
6) kontrola działania wojewódzkich organów nadzoru budowlanego.”,
b) dodaje 4 brzmieniu: lanych wydanych postanowieniach, decyzjach opiniach, przekazywanej przez wojewódzkich inspektorów nadzoru budowlanego Głównemu Inspektorowi Nadzoru Budowlanego,
...
c) po 2 dodaje 2a brzmieniu: „2a. Na zasadach określonych 25 2—6, można pobierać próbki budowlanego składowanych terenie budowy, jeżeli budowa prowadzona jest ramach działalności gospodarczej inwestora lub obiekt lany jest wznoszony celu wprowadzenia go do obrotu. Wyniki badań próbek wyrobu budowlanego mogą stanowić podstawę do wszczęcia kontroli.”;
7) 17:
a) 1 otrzymuje brzmienie: „1. Kontrole przeprowadza siedzibie kontrolowanego lub miejscu wykonywania jego działalności, czasie jej wykonywania obecności kontrolowanego lub osoby przez niego upoważnionej.”,
b) 3 otrzymuje brzmienie: „3. Kontrola dotyczy wyłącznie lanego, prawidłowości jego oznakowania oceny zgodności dokumentacji technicznej dotyczącej tego wyrobu.”;
8) 18 otrzymuje brzmienie: „Art. 18. 1. W trakcie kontroli kontrolujący może żądać kontrolowanego producenta albo importera przedstawienia wyznaczonym terminie:
1) deklaracji zgodności budowlanego;
...
3) legitymować osoby celu stwierdzenia ich tożsamości, jeżeli jest to niezbędne potrzeby kontroli;
4) żądać kontrolowanego udzielenia wyznaczonym pisemnych lub ustnych wyjaśnień sprawach objętych zakresem kontroli;
...
6) zasięgać opinii biegłych, jeżeli jest to niezbędne potrzeby kontroli;
7) zabezpieczać dowody, wyroby budowlane, pomieszczenia lub środki przewozowe;
nicą producenta, importera lub sprzedawcy, rozumiane jako nieujawnione do wiadomości publicznej informacje techniczne, technologiczne organizacyjne producenta, importera lub sprzedawcy bądź inne informacje, co do których producent, importer lub sprzedawca podjął niezbędne działania celu zachowania ich poufności, mogą być wykorzystane wyłącznie do celów, dla których zostały zgromadzone.”;
10) po 22b dodaje 22c brzmieniu: „Art. 22c. 1. Właściwy organ może, drodze postanowienia, dokonać zabezpieczenia budowlanego albo jego określonej partii przed dalszym przekazywaniem, jeżeli:
1) k ontrolowany nie przedstawi dokumentów związanych oceną zgodności budowlanego, albo
2) d okumenty wskazują, wyrób budowlany nie spełnia wymagań określonych niniejszą ustawą, albo
3) d okonane przez kontrolującego oględziny budowlanego wskazują, wyrób budowlany monie spełniać wymagań określonych niniejszą ustawą, albo
4) p osiadane przez organ (kontrolującego) dowody, rozumieniu przepisów Kodeksu postępowania administracyjnego, wskazują, właściwości użytkowe są niezgodne deklarowanymi.
2. Na postanowienie, którym mowa 1, przysługuje zażalenie.
3. Postanowienie, którym mowa 1, traci ważność po upływie 2 doręczenia kontrolowanemu.
4. Organ, który wydał I instancji postanowienie, którym mowa 1, stwierdza, drodze postanowienia, jego wygaśnięcie przed upływem 2 doręczenia, jeżeli ustały przyczyny uzasadniające zabezpieczenie budowlanego albo
...
19) 36 otrzymuje brzmienie: „Art. 36. Orzekanie sprawach określonych 35 następuje podstawie przepisów Kodeksu postępowania sprawach wykroczenia.”.

**Art. 2.** W ustawie sierpnia 2002 r. systemie oceny zgodności (Dz. U. 2004 r. 204, poz. 2087, późn. zm. ) wprowadza następujące zmiany:

**Art. 3.**

1. Do kontroli postępowań administracyjnych sprawie wszczętych niezakończonych przed dniem wejścia życie niniejszej ustawy stosuje Marszałek Sejmu wykonujący obowiązki przepisy dotychczasowe. Prezydenta Rzeczypospolitej Polskiej: B. Komorowski
